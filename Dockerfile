...
#--------------------------------------------------------------------------------
# Maintainer label and project source information
LABEL maintainer="Focela Labs (https://www.focela.com)"

#--------------------------------------------------------------------------------
# BUILD-TIME VERSION ARGUMENTS
#--------------------------------------------------------------------------------
# Arguments for component versions used at build time
...
#--------------------------------------------------------------------------------
# Default versions for all major components
...
RUN case "$(cat /etc/os-release | grep VERSION_ID | cut -d = -f 2 | cut -d . -f 1,2)" in \
    ...
    #--------------------------------------------------------------------------------
    # System-wide configuration and timezone setup
    apk add gettext && \
    ...
    #--------------------------------------------------------------------------------
    # Build and install OpenDoas from source if required
    ...
    #--------------------------------------------------------------------------------
    # Build and install Golang from source for Zabbix Agent 2
    ...
    #--------------------------------------------------------------------------------
    # Build and install yq YAML processor from source
    if [ "$yq" != "false" ] ; then \
    ...
    #--------------------------------------------------------------------------------
    # Build and install Zabbix agent and related components
    ...
    #--------------------------------------------------------------------------------
    # Build and install Fluent Bit for log processing
    ...
    #--------------------------------------------------------------------------------
    # Remove build dependencies and clean up image
    ...
    #--------------------------------------------------------------------------------
    # Install and configure S6 overlay for process supervision
    apkArch="$(apk --print-arch)"  && \
    case "$apkArch" in \
...
#--------------------------------------------------------------------------------
# Set shell, exposed ports, and entrypoint
SHELL ["/bin/bash", "-c"]
...
# APPLICATION FILES
#--------------------------------------------------------------------------------
# Copy application files and configuration into the image
COPY install/ /
